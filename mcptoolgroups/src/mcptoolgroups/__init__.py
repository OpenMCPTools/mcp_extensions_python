...
class Group(AbstractBase):
    ...
    def __str__(self):
        return f'name={self.name} fqname={self.fqname} parent={self.parent} title={self.title} description={self.description} icons={self.icons} annotations={self.annotations} meta={self.meta}'
...
class ToolGroupConverter():
    def convert_to(self, source):   
        parent = None
        # pydantic provides dict when deserializing Any
        if isinstance(source, dict):   
            parent = source.get('parent', None)
            g = Group(name=source.get('name'), 
                      title=source.get('title', None), 
                      description=source.get('description', None), 
                      meta=source.get('meta', None)) 
        else:
            parent = source.parent
            g = Group(name=source.name, 
                      title=source.title, 
                      description=source.description, 
                      meta=source.meta)
        if parent:
            self.convert_to(parent).add_child_group(g)
    ...
    def convert_from(self, target):
        ...
        if isinstance(target, dict):   
            parent = target.get('parent', None)
            g = GroupSchema(name=target.get('name'), 
                      title=target.get('title', None), 
                      description=target.get('description', None), 
                      meta=target.get('meta', None)) 
        else:
            parent = target.parent
            g = GroupSchema(name=target.name, 
                        title=target.title, 
                        description=target.description, 
                        meta=target.meta)
        if parent:
            g.parent = self.convert_from(parent)
        return g
